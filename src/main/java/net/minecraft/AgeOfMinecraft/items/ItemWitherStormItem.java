    ...
    if (!worldIn.isRemote) {
      ...
      while (i > 0) {
        ...
        if (!worldIn.getGameRules().getBoolean("disableExpItemDrops"))
          entityliving.world.spawnEntity((Entity)new EntityXPOrb(entityliving.world, entityliving.posX, entityliving.posY + entityliving.getEyeHeight() + 25.0D, entityliving.posZ, j)); 
      } 
    } 
