...
public class ModelEnderman extends ModelBiped implements ICappedModel {
  ...
  public ModelEnderman(float scale) {
    ...
    this.bipedRightLeg = new ModelRenderer((ModelBase)this, 56, 0);
  ...
  public void render(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
    setRotationAngles(limbSwing, limbSwingAmount, ((EntityTameBase)entityIn).isAIDisabled() ? 0.0F : ageInTicks, netHeadYaw, headPitch, scale, entityIn);
    GlStateManager.pushMatrix();
    if (this.isChild) {
  ...
  public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
    ...
    if (entity.getJukeboxToDanceTo() != null) {
      float fl = MathHelper.sin(ageInTicks * 0.5F) * 4.0F;
      this.bipedHead.setRotationPoint(0.0F, -16.0F - fl, 0.0F);
    } 
    ...
    if (!entity.getCurrentBook().isEmpty()) {
      this.bipedRightArm.rotateAngleY = (entity.bookSpread - 1.0F) * 0.25F;
      this.bipedLeftArm.rotateAngleY = (-entity.bookSpread + 1.0F) * 0.25F;
      this.bipedRightArm.rotateAngleZ = 0.0F;
      this.bipedLeftArm.rotateAngleZ = 0.0F;
      this.bipedRightArm.rotateAngleX = -1.25F + 0.1F + MathHelper.sin(entity.ticksExisted * 0.1F) * 0.01F;
      this.bipedLeftArm.rotateAngleX = -1.25F + 0.1F + MathHelper.sin(entity.ticksExisted * 0.1F) * 0.01F;
    } 
  }
}
