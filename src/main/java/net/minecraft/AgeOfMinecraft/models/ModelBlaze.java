...
public class ModelBlaze extends ModelBase implements ICappedModel {
  ...
  public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
    ...
    if (entity.getJukeboxToDanceTo() != null) {
      ...
      this.blazeHead.rotationPointX += MathHelper.cos(ageInTicks * 0.25F) * 1.0F;
      this.blazeHead.rotationPointY += f4 + MathHelper.cos(ageInTicks * 0.5F) * 1.0F;
      for (int m = 0; m < 12; m++) {
        f4 = MathHelper.sin(m * 2.0F + ageInTicks * 0.125F) * 0.7F;
        (this.blazeSticks[m]).rotationPointY += f4 + MathHelper.cos((m * 1.5F + ageInTicks) * 0.5F);
      } 
    } 
