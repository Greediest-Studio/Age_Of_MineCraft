...
public class ModelGuardian extends ModelBase {
  ...
  public void setRotationAngles(float p_78087_1_, float p_78087_2_, float p_78087_3_, float p_78087_4_, float p_78087_5_, float p_78087_6_, Entity p_78087_7_) {
    ...
    this.guardianBody.rotateAngleX = !entityguardian.isEntityAlive() ? 0.0F : (p_78087_5_ / 57.295776F);
    float[] afloat = { 
    ...
    float[] afloat5 = { 
        ...
        0.0F, 0.0F };
    float f7 = (1.0F - (entityguardian.isAIDisabled() ? 0.5F : entityguardian.getSpikesAnimation(f6))) * 0.55F;
    for (int i = 0; i < 12; i++) {
      ...
      (this.guardianSpines[i]).rotationPointX = afloat3[i] * (1.0F + MathHelper.cos((entityguardian.isAIDisabled() ? 0.0F : p_78087_3_) * 1.5F + i) * 0.01F - f7);
      (this.guardianSpines[i]).rotationPointY = afloat4[i] * (1.0F + MathHelper.cos((entityguardian.isAIDisabled() ? 0.0F : p_78087_3_) * 1.5F + i) * 0.01F - f7);
    ...
    if (object != null) {
      ...
      this.guardianEye.rotationPointX = entityguardian.isAIDisabled() ? 0.0F : (!entityguardian.getCurrentBook().isEmpty() ? (MathHelper.sin(p_78087_3_ * 0.1F + MathHelper.sin(p_78087_3_ * 0.05F)) * 1.5F) : (MathHelper.sqrt((float)Math.abs(d1)) * 2.0F * (float)Math.signum(d1)));
    } 
    ...
    if (entityguardian.getJukeboxToDanceTo() != null) {
      ...
      for (int k = 0; k < this.guardianTail.length; k++)
        (this.guardianTail[k]).rotateAngleY = MathHelper.sin((-1 - k) + (entityguardian.isAIDisabled() ? 1.0F : p_78087_3_) * 0.25F) * 0.5F; 
    } 
  }
}
