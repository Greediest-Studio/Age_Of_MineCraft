...
public class EntitySheep extends EntityTameBase implements Light, Animal, IShearable, IJumpingMount {
  ...
  public String getDescName() {
    return "jeb_".equals(getName()) ? "jeb_sheep" : ((getFleeceColor() == EnumDyeColor.PINK) ? "pink_sheep" : "sheep");
  }
  ...
  public void onLivingUpdate() {
    ...
    if (this.ticksExisted > 20 && hasCustomName() && "jeb_".equals(getCustomNameTag()) && getAttackTarget() != null) {
      if (this.shootTimer > 0) {
        getNavigator().clearPath();
        playSound(SoundEvents.BLOCK_LAVA_EXTINGUISH, getSoundVolume(), 0.5F + (this.shootTimer / 20));
      } 
  ...
  public boolean interact(EntityPlayer player, EnumHand hand) {
    ...
    if (!stack.isEmpty() && stack.getItem() == Items.DYE && hasOwner(player)) {
      ...
      if (!getSheared() && getFleeceColor() != enumdyecolor) {
        ...
        this.world.playEvent(2001, getPosition(), Block.getStateId(Blocks.CARPET.getDefaultState().withProperty((IProperty)BlockCarpet.COLOR, (Comparable)enumdyecolor)));
  ...
  public void travel(float strafe, float vertical, float forward) {
    if (isBeingRidden()) {
      ...
      if (this.jumpPower > 0.0F && this.onGround) {
        this.motionY = 0.7D * this.jumpPower;
  ...
  protected void playStepSound(BlockPos pos, Block blockIn) {
    playSound(SoundEvents.ENTITY_SHEEP_STEP, 0.15F, 1.0F / getFittness());
  }
