...
public class EntityAbomniableSnowman extends EntityTameBase implements IRangedAttackMob, Armored, Massive, Elemental {
  ...
  public void updatePassenger(Entity passenger) {
    if (isPassenger(passenger)) {
      ...
      if (i == 1)
        passenger.setPosition(this.posX - f4 * (1.2999999523162842D + ((this.limbSwingAmount >= 0.01D) ? (sho1 * 0.4D) : 0.0D)), this.posY + 3.5D, this.posZ - f11 * (1.2999999523162842D + ((this.limbSwingAmount >= 0.01D) ? (sho1 * 0.4D) : 0.0D))); 
      if (i == 0)
        passenger.setPosition(this.posX + f4 * (1.2999999523162842D + ((this.limbSwingAmount >= 0.01D) ? (sho1 * 0.4D) : 0.0D)), this.posY + 3.5D, this.posZ + f11 * (1.2999999523162842D + ((this.limbSwingAmount >= 0.01D) ? (sho1 * 0.4D) : 0.0D))); 
    } 
  }
  ...
  protected void onDeathUpdate() {
    ...
    playSound(SoundEvents.BLOCK_SNOW_BREAK, 1.0F, 1.0F);
  ...
  public void whirlSnowball(EntityLivingBase target) {
    ...
    double d1 = target.posX + this.rand.nextDouble() * 2.0D - 1.0D - this.posX + (f19 * 1.5F);
    double d2 = target.posY - 2.0D - this.rand.nextDouble() * 4.0D + target.height - this.posY + getEyeHeight();
    double d3 = target.posZ + this.rand.nextDouble() * 2.0D - 1.0D - this.posZ - (f3 * 1.5F);
  ...
  public void attackEntityWithRangedAttack(EntityLivingBase target, float p_82196_2_) {
    ...
    double d2 = target.posY - 3.0D + target.height - this.posY + getEyeHeight();
  ...
  public class AISpecialRangedAttack extends EntityAIBase {
    ...
    public void updateTask() {
      ...
      if (this.rangedAttackTime <= 0) {
        if (!flag)
          return; 
        if (this.entityHost.getHealth() <= this.entityHost.getMaxHealth() / 2.0F) {
