...
public class EntityEvokerFangOther extends EntityEvokerFangs {
  ...
  public void onUpdate() {
    ...
    if (this.world.isRemote) {
      ...
      if (this.warmupDelayTicks == -8)
        for (EntityLivingBase entitylivingbase : this.world.getEntitiesWithinAABB(EntityLivingBase.class, getEntityBoundingBox().grow(1.0D))) {
          if (getCaster() != null && getCaster() instanceof EntityTameBase && (!false || (entitylivingbase instanceof EntityTameBase && false && ((EntityTameBase)getCaster()).getFakeHealth() > 0.0F)))
  ...
  private void damage(EntityLivingBase p_190551_1_) {
    ...
    if (entitylivingbase == null)
      p_190551_1_.attackEntityFrom(DamageSource.MAGIC, 8.0F); 
  }
  ...
  public void handleStatusUpdate(byte id) {
    ...
    if (id == 4) {
      ...
      if (!isSilent())
        this.world.playSound(this.posX, this.posY, this.posZ, SoundEvents.EVOCATION_FANGS_ATTACK, getSoundCategory(), 1.0F, this.rand.nextFloat() * 0.2F + 0.85F, false); 
    } 
  }
