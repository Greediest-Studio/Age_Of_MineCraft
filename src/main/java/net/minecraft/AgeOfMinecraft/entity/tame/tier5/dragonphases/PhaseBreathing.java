...
public class PhaseBreathing extends PhaseBaseFriendly {
  ...
  public void doClientRenderEffects() {
    ...
    if (this.flameTicks < 30) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        for (int j = 0; j < 4; j++)
          this.dragon.world.spawnParticle(EnumParticleTypes.DRAGON_BREATH, d3, d4, d5, -vec3d.x * 0.1D * j, -vec3d.y * 0.75D, -vec3d.z * 0.1D * j, new int[0]); 
  ...
  public void doLocalUpdate() {
    ...
    this.flameTicks++;
    if (this.flameTicks < 30)
      this.dragon.playSound(SoundEvents.ENTITY_ENDERDRAGON_SHOOT, 5.0F, 0.5F); 
