...
public class BlockMonsterSpawnerSPC extends BlockContainer {
  public BlockMonsterSpawnerSPC() {
    super(Material.IRON);
    setTranslationKey("mob_spawner_spc");
    setHardness(5.0F);
  ...
  public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
    ...
    TileEntity tileentity = worldIn.getTileEntity(pos);
    if (tileentity instanceof TileEntityMonsterSpawnerSPC)
      FMLNetworkHandler.openGui(playerIn, EngenderMod.instance, 101, playerIn.world, pos.getX(), pos.getY(), pos.getZ()); 
    return true;
  }
  ...
  public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
    ...
    if (tileentity instanceof TileEntityMonsterSpawnerSPC) {
      ...
      if (((TileEntityMonsterSpawnerSPC)tileentity).mana > 0.0D)
        worldIn.spawnEntity((Entity)new EntityManaOrb(worldIn, pos.getX(), pos.getY(), pos.getZ(), (int)((TileEntityMonsterSpawnerSPC)tileentity).mana, false)); 
      if (((TileEntityMonsterSpawnerSPC)tileentity).entropy > 0.0D)
        worldIn.spawnEntity((Entity)new EntityManaOrb(worldIn, pos.getX(), pos.getY(), pos.getZ(), (int)((TileEntityMonsterSpawnerSPC)tileentity).entropy, true)); 
    } 
  ...
  public Item getItemDropped(IBlockState state, Random rand, int fortune) {
    return Item.getItemFromBlock((Block)EBlock.mob_spawner_spc);
  }
}
