...
public class TileEntityBeaconSPC extends TileEntityLockable implements ITickable, ISidedInventory {
  ...
  private void updateSegmentColors() {
    ...
    while (true) {
      if (i1 < 256) {
        float[] afloat;
        IBlockState iblockstate = this.world.getBlockState((BlockPos)blockpos$mutableblockpos.setPos(i, i1, k));
        if (iblockstate.getBlock() == Blocks.STAINED_GLASS) {
          afloat = ((EnumDyeColor)iblockstate.getValue((IProperty)BlockStainedGlass.COLOR)).getColorComponentValues();
        } else if (iblockstate.getBlock() != Blocks.STAINED_GLASS_PANE) {
          if (iblockstate.getLightOpacity((IBlockAccess)this.world, (BlockPos)blockpos$mutableblockpos) >= 15 && iblockstate.getBlock() != Blocks.BEDROCK) {
            this.isComplete = false;
            this.beamSegments.clear();
            break;
          } 
          float[] customColor = iblockstate.getBlock().getBeaconColorMultiplier(iblockstate, this.world, (BlockPos)blockpos$mutableblockpos, getPos());
          if (customColor != null) {
            afloat = customColor;
          } else {
          ...
          afloat = ((EnumDyeColor)iblockstate.getValue((IProperty)BlockStainedGlassPane.COLOR)).getColorComponentValues();
        } 
        if (!flag)
      ...
      for (int l1 = 1; l1 <= 4; this.levels = l1++) {
        ...
        for (int j1 = i - l1; j1 <= i + l1 && flag1; j1++) {
          for (int k1 = k - l1; k1 <= k + l1; k1++) {
            ...
            if (!block.isBeaconBase((IBlockAccess)this.world, new BlockPos(j1, i2, k1), getPos())) {
              flag1 = false;
              break;
            } 
          } 
