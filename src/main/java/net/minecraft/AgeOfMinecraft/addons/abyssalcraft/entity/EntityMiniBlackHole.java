    ...
    if (list != null && !list.isEmpty())
      for (int i = 0; i < list.size(); i++) {
        ...
        if (entity.isEntityAlive() && !(entity instanceof net.minecraft.AgeOfMinecraft.entity.tame.tier6.EntityWitherStorm) && !(entity instanceof EntityJzahar) && this.shootingEntity != null && entity instanceof EntityLivingBase && !false) {
          ...
          if (entity.getDistanceSq((Entity)this) <= 9.0D && this.shootingEntity != null && entity instanceof EntityLivingBase && !false) {
            ...
            if (EngenderConfig.general.useMessage && entity instanceof EntityLivingBase && !entity.isEntityAlive() && !this.shootingEntity.isWild())
              this.shootingEntity.getOwner().sendMessage((ITextComponent)new TextComponentTranslation(entity.getName() + " touched a Mini Black Hole's event horizon thanks to " + this.shootingEntity.getName() + " (" + this.shootingEntity.getOwner().getName() + ")", new Object[0])); 
          } 
          if (entity.getDistanceSq((Entity)this) <= 2.0D && this.shootingEntity != null && entity instanceof EntityLivingBase && !false) {
            ...
            if (EngenderConfig.general.useMessage && entity instanceof EntityLivingBase && !entity.isEntityAlive() && !this.shootingEntity.isWild())
              this.shootingEntity.getOwner().sendMessage((ITextComponent)new TextComponentTranslation(entity.getName() + " was sucked into a Mini Black Hole thanks to " + this.shootingEntity.getName() + " (" + this.shootingEntity.getOwner().getName() + ")", new Object[0])); 
        ...
        if (entity != null && !(entity instanceof EntityLivingBase) && !(entity instanceof EntityMiniBlackHole) && !(entity instanceof EntityBlackHole) && !(entity instanceof EntityImplosion)) {
          ...
          if (entity instanceof net.minecraft.AgeOfMinecraft.addons.draconicevolution.EntityGuardianProjectile && getDistance(entity) <= 1.0D && this.shootingEntity != null && this.shootingEntity instanceof EntityTameBase) {
            EntityTameBase.createEngenderModExplosion((Entity)this.shootingEntity, entity.posX, entity.posY, entity.posZ, 2.0F, true, false);
            entity.setDead();
          } 
        } 
      }  
